...
import json
import os
from urllib.parse import urlparse, quote, parse_qs , urlencode # Ensure urlencode is here
...
# Make sure FileResponse is imported if you use it for local file streaming
...
from django.views.decorators.csrf import csrf_exempt, csrf_protect # csrf_protect for add_review
...
from .utils import _get_full_context # Assuming _get_full_context is in AudioXApp/views/utils.py
...
def parse_duration_to_seconds(duration_str):
    ...
    try:
        ...
        elif len(parts) == 1: # Seconds only (can be float string)
            return int(float(duration_str))
    except ValueError:
        # If split by ':' fails or int/float conversion fails, it might be just seconds as a string.
        # Fallback or further parsing can be added if other formats are common.
        pass # Let it try other parsing or return None

    # If it's just a number (possibly with decimals) representing seconds
    try:
        return int(float(duration_str))
    except ValueError:
        return None
...
def fetch_audiobooks_data():
    ...
    """
    cache_key = 'librivox_archive_audiobooks_data_v7' # Incremented cache key version
    ...
    for rss_url in rss_feeds:
        try:
            ...
            chapters_data_for_book = [] # Renamed from chapters_data
            for entry in feed.entries:
                audio_url = None
                # ... (your existing logic for finding audio_url) ...
                if entry.enclosures:
                ...
                # --- ADDED: Attempt to parse LibriVox chapter duration ---
                chapter_duration_seconds = None
                if hasattr(entry, 'itunes_duration'):
                    chapter_duration_seconds = parse_duration_to_seconds(entry.itunes_duration)
                # --- END ADDED ---

                chapters_data_for_book.append({ # Appending to renamed list
                    "chapter_title": chapter_title, 
                    "audio_url": audio_url,
                    "duration_seconds": chapter_duration_seconds # ADDED
                })

            if not chapters_data_for_book: # Check renamed list
                logger.info(f"No chapters with audio found for audiobook in {rss_url}")
                continue

            # ... (your existing logic for title, description, author, cover_image_original_url) ...
            ...
            cover_image_original_url = None # Your existing logic to find this
            if hasattr(feed.feed, 'image') and hasattr(feed.feed.image, 'href'):
                cover_image_original_url = feed.feed.image.href
            # ... (ensure all your cover image fallbacks are here) ...
            ...
            first_chapter_original_audio_url = chapters_data_for_book[0]["audio_url"] if chapters_data_for_book else None # Use renamed list
            first_chapter_title = chapters_data_for_book[0]["chapter_title"] if chapters_data_for_book else None # Use renamed list
            ...
            librivox_audiobooks.append({
                ...
                "chapters": chapters_data_for_book, # Use renamed list
    ...
    archive_timeout = 30 
    term_index_api_search = 0 # You had this, ensure it's used if intended for rate limiting

    for term in search_terms:
        # ... (your existing logic for term_index_api_search and query_string) ...
        if term_index_api_search > 0: time.sleep(1) # Simple delay
        ...
        try:
            ...
            for doc_api_data in docs:
                ...
                # ... (your existing logic for doc_title, author_from_doc, etc.) ...
                ...
                chapters_from_api = [] # Renamed from chapters
                for f_item in files_metadata_api:
                    if f_item.get("format") in ["VBR MP3", "MP3", "64Kbps MP3", "128Kbps MP3"] and "name" in f_item:
                        ...
                        # --- ADDED: Attempt to parse Archive.org chapter duration ---
                        chapter_duration_seconds = None
                        # Archive.org often provides 'length' in seconds as a string, or 'duration'
                        raw_length = f_item.get('length') or f_item.get('duration')
                        if raw_length:
                            chapter_duration_seconds = parse_duration_to_seconds(raw_length)
                        # --- END ADDED ---
                        
                        chapters_from_api.append({ # Appending to renamed list
                            "chapter_title": chapter_title,
                            "audio_url": f"https://archive.org/download/{identifier}/{quote(f_item['name'])}",
                            "duration_seconds": chapter_duration_seconds # ADDED
                        })
                
                if not chapters_from_api: # Check renamed list
                ...
                book_data = {
                    ...
                    "chapters": chapters_from_api, # Use renamed list
                    "first_chapter_audio_url": chapters_from_api[0]["audio_url"] if chapters_from_api else None, # Use renamed list
                    "first_chapter_title": chapters_from_api[0]["chapter_title"] if chapters_from_api else None, # Use renamed list
            ...
            if audiobooks_for_term:
                # ... (your existing logic to add to archive_language_audiobooks or archive_genre_audiobooks) ...
                if term in language_specific_terms:
    ...
    manual_language_items = { # Your manual items list
        "Urdu": [
    ...
    for lang_key, item_ids_list in manual_language_items.items():
        # ... (your existing logic for manual items, but ADD DURATION EXTRACTION) ...
        ...
        for item_identifier in item_ids_list:
            ...
            try:
                time.sleep(0.5) # Your delay
                meta_url = f"https://archive.org/metadata/{item_identifier}"
                meta_resp = session.get(meta_url, timeout=archive_timeout, headers=headers)
                meta_resp.raise_for_status()
                item_full_metadata = meta_resp.json()
                # ... (your existing logic for doc_title_manual, author_from_doc_manual, etc.) ...
                ...
                # ... (rest of your metadata extraction for manual item) ...
                ...
                chapters_for_manual_item = [] # Renamed
                for f_item_manual in files_metadata_manual:
                    if f_item_manual.get("format") in ["VBR MP3", "MP3", "64Kbps MP3", "128Kbps MP3"] and "name" in f_item_manual:
                        ...
                        # --- ADDED: Attempt to parse Archive.org chapter duration for manual items ---
                        ...
                        chapters_for_manual_item.append({ # Appending to renamed list
                            "chapter_title": chapter_title_manual,
                            "audio_url": f"https://archive.org/download/{item_identifier}/{quote(f_item_manual['name'])}",
                            "duration_seconds": manual_chapter_duration_seconds # ADDED
                        })
                if not chapters_for_manual_item: # Check renamed list
                ...
                book_data_manual = {
                    ...
                    "chapters": chapters_for_manual_item, # Use renamed list
                    "first_chapter_audio_url": chapters_for_manual_item[0]["audio_url"] if chapters_for_manual_item else None, # Use renamed list
                    "first_chapter_title": chapters_for_manual_item[0]["chapter_title"] if chapters_for_manual_item else None, # Use renamed list
    ...
    if fetch_successful:
        ...
        # ... (your existing logging for final counts) ...
        ...
        # ... (your existing logging for unsuccessful fetch) ...
...
def api_audiobooks(request):
    cache_key = 'librivox_archive_audiobooks_data_v7'  # UPDATED from v6 to v7
...
def home(request):
    ...
    if audiobook_data_from_cache:
        ...
        for lang_key, book_list_lang in audiobook_data_from_cache.get("archive_language_audiobooks", {}).items(): # Corrected variable name
    ...
    if all_external_book_slugs:
        db_instances = Audiobook.objects.filter(
            slug__in=list(all_external_book_slugs),
            is_creator_book=False 
        )
        for db_book in db_instances:
            db_book_stats[db_book.slug] = {
                ...
                'title': db_book.title,    
                'cover_image': db_book.cover_image.url if db_book.cover_image else None 
            }
    ...
    if audiobook_data_from_cache:
        ...
        logger.info(f"DEBUG: Initial number of genres in cached_archive_genres: {len(cached_archive_genres)}")
        if not cached_archive_genres:
             logger.warning("DEBUG: 'archive_genre_audiobooks' is empty or not found in cache.")

        for genre, book_list in cached_archive_genres.items():
            logger.info(f"DEBUG: Processing genre: '{genre}', Number of books initially from cache: {len(book_list)}")
            english_books_for_genre = []
            for i, book_data in enumerate(book_list): 
                if i < 3: 
                    logger.info(f"DEBUG: Book {i+1} in genre '{genre}': Title='{book_data.get('title')}', Raw Language field from cache='{book_data.get('language')}'")
                
                lang = book_data.get('language', 'English') 
                is_english = False
                # --- CORRECTED LOGIC FOR is_english ---
                if isinstance(lang, list): 
                    is_english = any(str(l).lower().strip() in ['english', 'en', 'eng'] for l in lang)
                elif isinstance(lang, str): 
                    is_english = lang.lower().strip() in ['english', 'en', 'eng'] # Added 'eng'
                # --- END CORRECTION ---
                
                if i < 3: 
                     logger.info(f"DEBUG: Book {i+1} in genre '{genre}': Parsed lang='{lang}', Evaluated is_english='{is_english}'")
            ...
            logger.info(f"DEBUG: Genre '{genre}' - Number of English books identified and processed: {len(english_books_for_genre)}")
            if english_books_for_genre:
                archive_genre_audiobooks_final[genre] = english_books_for_genre
            elif book_list : 
                logger.warning(f"DEBUG: Genre '{genre}' had {len(book_list)} books from cache, but none were identified as English after filtering.")
        ...
        logger.warning("DEBUG: Audiobook data cache was empty or not found.")

    context["librivox_audiobooks"] = librivox_audiobooks_final
    context["archive_genre_audiobooks"] = archive_genre_audiobooks_final
    logger.info(f"DEBUG: Final 'archive_genre_audiobooks' being passed to template has {len(archive_genre_audiobooks_final)} genres: {list(archive_genre_audiobooks_final.keys())}")
    if not archive_genre_audiobooks_final and cached_archive_genres : # Check if original cache had items
        logger.warning("DEBUG: 'archive_genre_audiobooks_final' is empty, but 'cached_archive_genres' was not. This suggests the English filter removed all items.")
    ...
    try:
        ...
        for book in creator_books_qs:
            ...
            creator_books_list.append({
                ...
                'first_chapter_title': first_chapter_title, 'is_creator_book': True, 
                'average_rating': book.average_rating, 
                'total_views': book.total_views, 
                'is_paid': book.is_paid, 'price': book.price, 'status': book.status, 
                'language': book.language, 'genre': book.genre
            })
...
def _render_genre_or_language_page(request, page_type, display_name, template_name, cache_key_segment, query_term, language_for_genre_page=None):
    ...
    if external_book_slugs_from_cache:
        db_instances = Audiobook.objects.filter(
            slug__in=list(external_book_slugs_from_cache),
            is_creator_book=False
        )
        for db_book in db_instances:
            db_book_stats_map[db_book.slug] = {
                'total_views': db_book.total_views,
                'average_rating': db_book.average_rating,
                'author': db_book.author,
                'title': db_book.title,
    ...
    try:
        ...
        for book in creator_books_qs:
            ...
            creator_books_final_list.append({
                'source': 'creator', 'title': book.title, 'slug': book.slug, 
                'cover_image': book.cover_image.url if book.cover_image else None, 
                'author': book.author, 'creator': book.creator, 
                'first_chapter_audio_url': first_chapter_audio_url, 
                ...
                'is_paid': book.is_paid, 'price': book.price, 
                'status': book.status, 'language': book.language, 'genre': book.genre
            })
...
# --- NEW LANGUAGE-SPECIFIC GENRE VIEWS ---
# Urdu Genres
def urdu_genre_novel_afsana(request): 
    return _render_genre_or_language_page(
        ...
        "genre", 
        "Urdu Novel & Afsana", 
        'audiobooks/Urdu/genre_novel_afsana.html', 
        "archive_language_audiobooks", 
        "Novel Afsana",  # CORRECTED: Was "Novel / Afsana"
        language_for_genre_page="Urdu"
    )
...
def urdu_genre_shayari(request): 
    return _render_genre_or_language_page(
        ...
        "genre", 
        "Urdu Shayari", 
        'audiobooks/Urdu/genre_shayari.html', 
        "archive_language_audiobooks", 
        "Shayari", 
        language_for_genre_page="Urdu"
    )
...
def urdu_genre_tareekh(request): 
    return _render_genre_or_language_page(
        ...
        "genre", 
        "Urdu Tareekh", 
        'audiobooks/Urdu/genre_tareekh.html', 
        "archive_language_audiobooks", 
        "Tareekh", 
        language_for_genre_page="Urdu"
    )
...
def urdu_genre_safarnama(request): 
    return _render_genre_or_language_page(
        ...
        "genre", 
        "Urdu Safarnama", 
        'audiobooks/Urdu/genre_safarnama.html', 
        "archive_language_audiobooks", 
        "Safarnama", 
        language_for_genre_page="Urdu"
    )
...
def urdu_genre_mazah(request): 
    return _render_genre_or_language_page(
        ...
        "genre", 
        "Urdu Mazah", 
        'audiobooks/Urdu/genre_mazah.html', 
        "archive_language_audiobooks", 
        "Mazah", 
        language_for_genre_page="Urdu"
    )
...
def urdu_genre_bachon_ka_adab(request): 
    return _render_genre_or_language_page(
        ...
        "genre", 
        "Urdu Bachon ka Adab", 
        'audiobooks/Urdu/genre_bachon_ka_adab.html', 
        "archive_language_audiobooks", 
        "Bachon ka Adab", 
        language_for_genre_page="Urdu"
    )
...
def urdu_genre_mazhabi_adab(request): 
    return _render_genre_or_language_page(
        ...
        "genre", 
        "Urdu Mazhabi Adab", 
        'audiobooks/Urdu/genre_mazhabi_adab.html', 
        "archive_language_audiobooks", 
        "Mazhabi Adab", 
        language_for_genre_page="Urdu"
    )
...
def punjabi_genre_qissalok(request): 
    return _render_genre_or_language_page(
        ...
        "genre", 
        "Punjabi Qissa Lok", 
        'audiobooks/Punjabi/genre_qissalok.html', 
        "archive_language_audiobooks", 
        "Qissalok",  # CORRECTED: Was "Qissa Lok"
        language_for_genre_page="Punjabi"
    )
...
def punjabi_genre_geet(request): 
    return _render_genre_or_language_page(
        ...
        "genre", 
        "Punjabi Geet", 
        'audiobooks/Punjabi/genre_geet.html', 
        "archive_language_audiobooks", 
        "Geet", 
        language_for_genre_page="Punjabi"
    )
...
def sindhi_genre_lok_adab(request): 
    return _render_genre_or_language_page(
        ...
        "genre", 
        "Sindhi Lok Adab", 
        'audiobooks/Sindhi/genre_lok_adab.html', 
        "archive_language_audiobooks", 
        "Lok Adab", 
        language_for_genre_page="Sindhi"
    ) 
...
def sindhi_genre_shayari(request): 
    return _render_genre_or_language_page(
        ...
        "genre", 
        "Sindhi Shayari", 
        'audiobooks/Sindhi/genre_shayari.html', 
        "archive_language_audiobooks", 
        "Shayari", 
        language_for_genre_page="Sindhi"
    )
# --- END NEW LANGUAGE-SPECIFIC GENRE VIEWS --- 
...
def audiobook_detail(request, audiobook_slug):
    ...
    # MODIFICATION: Always use audiobook_detail.html
        ...
        if found_external_book_dict:
            ...
            audiobook_obj, created = Audiobook.objects.get_or_create(
                ...
                defaults={
                    ...
                    'is_creator_book': False, 'creator': None,
                    'is_paid': False, 'price': Decimal('0.00'), 'status': 'PUBLISHED',
                }
            )
            if created:
    ...
    if is_creator_book_page_flag and audiobook_obj:
        ...
        # MODIFICATION: Removed this line as we always use 'audiobook_detail.html'
        ...
        # For external books, the DB object's average_rating and total_views are the source of truth
        ...
        context_audiobook_data_for_display = audiobook_obj 
        is_creator_book_page_flag = audiobook_obj.is_creator_book # Ensure this flag is set if audiobook_obj exists but found_external_book_dict doesn't
    ...
    if audiobook_obj:
        ...
        audiobook_obj.refresh_from_db(fields=['total_views']) # Refresh to get the latest total_views

        # Update display data if it's a dict (for external books)
        if isinstance(context_audiobook_data_for_display, dict):
            context_audiobook_data_for_display['total_views'] = audiobook_obj.total_views
            # The average_rating for external books display should also come from the refreshed audiobook_obj
            context_audiobook_data_for_display['average_rating'] = audiobook_obj.average_rating 
    ...
    if audiobook_obj and is_creator_book_page_flag:
        all_db_chapters = audiobook_obj.chapters.all() # chapters are prefetched
        ...
        for i, chapter_db_obj in enumerate(all_db_chapters):
            ...
            chapters_to_display.append({
                ...
                'audio_url_template': chapter_db_obj.get_streaming_url(),
                'is_accessible': is_accessible,
                'duration_seconds': chapter_db_obj.duration_seconds,
                'chapter_index': i,
                'chapter_id': str(chapter_db_obj.chapter_id) 
            })
    elif found_external_book_dict: # External book
        ...
        for i, ch_info in enumerate(external_chapters_raw):
            ...
            if external_audio_url:
                # Ensure quote is imported from urllib.parse
            ...
            chapters_to_display.append({
                ...
                'audio_url_template': streaming_url_template,
                'is_accessible': True, 
                'duration_seconds': ch_info.get('duration_seconds'),
                'chapter_index': i,
                'chapter_id': f"ext-{audiobook_obj.slug}-{i}" if audiobook_obj else f"ext-unknown-{i}" 
            })

    page_context_for_js = {
        "isAuthenticated": request.user.is_authenticated,
        "csrfToken": get_token(request),
        "audiobookId": str(audiobook_obj.audiobook_id) if audiobook_obj and audiobook_obj.audiobook_id else None,
        "audiobookSlug": audiobook_obj.slug if audiobook_obj else None,
        "audiobookTitle": audiobook_obj.title if audiobook_obj else (found_external_book_dict.get('title', "Audiobook") if found_external_book_dict else "Audiobook"),
        "addReviewUrl": reverse('AudioXApp:add_review', kwargs={'audiobook_slug': audiobook_obj.slug}) if audiobook_obj else "#error-no-slug-for-review",
        "addToLibraryApiUrl": reverse('AudioXApp:toggle_library_item'),
        "myLibraryUrl": reverse('AudioXApp:my_library_page'),
        "loginUrl": f"{reverse('AudioXApp:login')}?next={urlencode({'next': request.path})}", # Ensure urlencode is imported
        "getAiSummaryUrl": reverse('AudioXApp:get_ai_summary', kwargs={'audiobook_id': audiobook_obj.audiobook_id}) if audiobook_obj and audiobook_obj.audiobook_id else reverse('AudioXApp:get_ai_summary', kwargs={'audiobook_id': 0}),
        "updateListeningProgressUrl": reverse('AudioXApp:update_listening_progress'), 
        "postChapterCommentUrlBase": "URL_STILL_NEEDS_DEFINITION_post_chapter_comment", 
        "getChapterCommentsUrlBase": "URL_STILL_NEEDS_DEFINITION_get_chapter_comments", 
        # ADDED for JS logic that expects these for creator books
        "audiobookAuthor": audiobook_obj.author if audiobook_obj else (found_external_book_dict.get('author') if found_external_book_dict else "Unknown Author"),
        "audiobookLanguage": audiobook_obj.language if audiobook_obj else (found_external_book_dict.get('language') if found_external_book_dict else "N/A"),
        "audiobookGenre": audiobook_obj.genre if audiobook_obj else (found_external_book_dict.get('genre') if found_external_book_dict else "N/A"),
        "isCreatorBook": is_creator_book_page_flag, # Use the determined flag
        "generateAudioClipUrl": reverse('AudioXApp:generate_audio_clip'), # Make sure this is defined in urls.py
        # --- ADDED STRIPE CONFIG FOR JS ---
        "stripePublishableKey": settings.STRIPE_PUBLISHABLE_KEY if hasattr(settings, 'STRIPE_PUBLISHABLE_KEY') else None,
        "createCheckoutSessionUrl": reverse('AudioXApp:create_checkout_session') if is_creator_book_page_flag and audiobook_obj and audiobook_obj.is_paid else None, # Only if it's a paid creator book
        "audiobookPrice": str(audiobook_obj.price) if audiobook_obj and audiobook_obj.is_paid else "0.00",
        # --- END ADDED STRIPE CONFIG ---
    }
    if request.user.is_authenticated:
    ...
    # 1. Recommend Creator Books from DB
    if current_book_language:
        # Base query for DB recommendations with annotation
        db_recs_query_base = Audiobook.objects.filter(
            status='PUBLISHED',
            is_creator_book=True,
            language__iexact=current_book_language
        ).exclude(slug__in=list(seen_recommended_slugs)).annotate(
            calculated_average_rating=Avg('reviews__rating') # ANNOTATION ADDED
        )

        if current_book_genre:
            genre_specific_db_recs = list(
                db_recs_query_base.filter(genre__iexact=current_book_genre)
                .order_by('-calculated_average_rating', '?')[:max_recommendations] # ORDER BY ANNOTATION
            )
        ...
        if len(recommended_audiobooks_list) < max_recommendations:
            ...
            other_lang_db_recs = list(
                db_recs_query_base.exclude(exclude_q)
                .order_by('-calculated_average_rating', '?')[:max_recommendations - len(recommended_audiobooks_list)] # ORDER BY ANNOTATION
            )
    ...
    # 2. Recommend External Audiobooks from Cache
    if len(recommended_audiobooks_list) < max_recommendations and current_book_language:
        ...
        if external_audiobook_data_cache:
            ...
            if isinstance(archive_genre_books, dict):
                for genre_key, book_list in archive_genre_books.items(): # genre_key might not be needed if filtering by lang
                    for book_dict in book_list:
                        book_lang_ext = book_dict.get('language')
                        if book_lang_ext and current_book_language.lower() in book_lang_ext.lower() and book_dict.get('slug') not in seen_recommended_slugs:
                            # Add only if not already added to avoid duplicates from different cache sections
            ...
            if current_book_genre:
                ...
                for slug, book_dict_item in unique_external_recs_by_slug.items():
                    ...
                    genre_match_found = (book_g_ext and current_book_genre.lower() in book_g_ext.lower()) or \
                                   any(current_book_genre.lower() in s.lower() for s in book_s_ext if isinstance(s, str))
            ...
            for book_dict_ext in sorted_external_recs:
                ...
                if ext_slug and ext_slug not in seen_recommended_slugs:
                    ...
                    # Enrich external book dict with DB stats if a shell object exists for it
                    # This uses the main audiobook_obj which corresponds to the current external book's shell
                    # We need to fetch stats for EACH recommended external book IF they have a shell
                    # This could be optimized by fetching all relevant shells in one go if performance is an issue
                    try:
                        shell_for_rec = Audiobook.objects.get(slug=ext_slug, is_creator_book=False)
                        book_dict_ext['average_rating'] = shell_for_rec.average_rating # Uses the property
                        book_dict_ext['total_views'] = shell_for_rec.total_views
                    except Audiobook.DoesNotExist:
                        book_dict_ext['average_rating'] = None # Default if no shell
    ...
    context['audiobook'] = audiobook_obj # Pass the DB object for reviews, etc.
    context['audiobook_data_for_display'] = context_audiobook_data_for_display # This is what the template will primarily use for display
    context['is_creator_book_page'] = is_creator_book_page_flag # Crucial for template logic
...
def add_review(request, audiobook_slug):
    try:
        ...
        # Determine if user can review (current logic for can_review seems okay)
        if not audiobook.is_paid:
            can_review = True
        elif audiobook.is_paid:
            if audiobook.is_creator_book: # Only check purchase for creator books
                if request.user.is_authenticated and hasattr(request.user, 'has_purchased_audiobook'):
                    if request.user.has_purchased_audiobook(audiobook):
                        can_review = True
            else: # External books (Librivox, Archive.org) are typically free even if mirrored
                can_review = True 
        ...
        # Assuming your User model has profile_pic and full_name.
        ...
        review_data = {
            ...
            'user_id': review.user.user_id, # Directly use user_id from your User model
            'user_name': review.user.full_name or review.user.username, # Prefer full_name
        ...
        return JsonResponse({
            ...
            'message': message, 
            'created': created, 
            'new_average_rating': str(new_average_rating_val) if new_average_rating_val is not None else "0.0", 
...
@csrf_exempt # Keep if POST might be used, though GET is typical for streaming
@require_GET # Streaming should be GET
def stream_audio(request):
    ...
    if target_audio_url.startswith(settings.MEDIA_URL):
        # Further check: does it look like a relative path from MEDIA_URL?
        # A simple check: no scheme and no netloc usually means it's a path relative to the current host.
        if not parsed_url.scheme and not parsed_url.netloc:
            is_local_media = True

    if is_local_media:
        try:
            # Construct filesystem path from MEDIA_URL and MEDIA_ROOT
            # Example: audio_url_param might be /media/chapters_audio/koko/ch_1.mp3
            # We need chapters_audio/koko/ch_1.mp3
            ...
            # Use FileResponse for efficient local file streaming with range request support
            # FileResponse handles Content-Type, Content-Length, and Range requests automatically.
            logger.info(f"Streaming local file: {file_system_path}")
            return FileResponse(open(file_system_path, 'rb'), as_attachment=False) # as_attachment=False for inline playback
    ...
    # If not local media, assume it's an external URL and proceed with requests proxy
    elif not all([parsed_url.scheme, parsed_url.netloc]): # Check if it's a valid-looking absolute URL
        ...
        try:
            ...
            # Increased timeout for potentially large audio files from external sources
            audio_stream_timeout = (10, 60) # (connect_timeout, read_timeout)

            response_ext = requests.get(target_audio_url, stream=True, headers=proxy_headers, timeout=audio_stream_timeout)
            response_ext.raise_for_status() # Will raise HTTPError for bad responses (4xx or 5xx)
            ...
            # response.iter_content to stream, Django StreamingHttpResponse will handle it
            streaming_response = StreamingHttpResponse(
                response_ext.iter_content(chunk_size=8192),
                content_type=content_type,
                status=response_ext.status_code # Pass original status code
            )

            # Copy relevant headers for streaming (Range requests, content length)
        ...
        except SuspiciousOperation as e_susp: # Should not happen with GET
