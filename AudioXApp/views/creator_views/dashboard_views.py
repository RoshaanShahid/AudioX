...
from ...models import Creator, Audiobook, Chapter, WithdrawalRequest, WithdrawalAccount, CreatorEarning
...
def creator_dashboard_view(request):
    ...
    try:
        ...
        sales_earnings_aggregation = CreatorEarning.objects.filter(creator=creator_profile, earning_type='sale').aggregate(total_sales_earn=Sum('amount_earned'))
        sales_earnings = sales_earnings_aggregation['total_sales_earn'] or Decimal('0.00')

        view_earnings_aggregation = CreatorEarning.objects.filter(creator=creator_profile, earning_type='view').aggregate(total_view_earn=Sum('amount_earned'))
        ...
        total_withdrawn_aggregation = WithdrawalRequest.objects.filter(creator=creator_profile, status='completed').aggregate(total_withdrawn=Sum('amount'))
        ...
        if top_performing_book_obj:
            best_performing_book_info = {'title': top_performing_book_obj.title, 'listens': top_performing_book_obj.total_views or 0}

        recent_activities = []
        latest_book_uploads = Audiobook.objects.filter(creator=creator_profile).order_by(Coalesce('publish_date', 'created_at', Value(now - timedelta(days=365*10))))[:5]
        for book in latest_book_uploads:
            ...
            if ts:
                recent_activities.append({'type': 'audiobook_upload', 'icon_class': 'fas fa-book-medical', 'description': f"Audiobook '{book.title}' published.", 'timestamp': ts, 'url': reverse('AudioXApp:creator_manage_upload_detail', args=[book.slug]) if book.slug else '#'})

        if hasattr(Audiobook, 'updated_at') and hasattr(Audiobook, 'created_at'):
            recent_book_edits = Audiobook.objects.filter(creator=creator_profile).annotate(effective_creation_date=Coalesce(F('publish_date'), F('created_at'))).filter(updated_at__gt=F('effective_creation_date') + timedelta(minutes=1)).order_by('-updated_at')[:5]
            for book in recent_book_edits:
                if book.updated_at:
                    recent_activities.append({'type': 'audiobook_edit', 'icon_class': 'fas fa-edit', 'description': f"Details for '{book.title}' updated.", 'timestamp': book.updated_at, 'url': reverse('AudioXApp:creator_manage_upload_detail', args=[book.slug]) if book.slug else '#'})
        ...
        if hasattr(WithdrawalAccount, 'added_at'):
            ...
            for acc in recent_accounts_added:
                if acc.added_at:
                    recent_activities.append({'type': 'payment_account_add', 'icon_class': 'fas fa-university', 'description': f"{acc.get_account_type_display()} account added.", 'timestamp': acc.added_at, 'url': reverse('AudioXApp:creator_manage_withdrawal_accounts')})
        ...
        for wr in latest_withdrawals:
            if wr.request_date:
                recent_activities.append({'type': 'withdrawal_request', 'icon_class': 'fas fa-hand-holding-usd', 'description': f"Withdrawal for Rs. {wr.amount:.2f} ({wr.get_status_display()}).", 'timestamp': wr.request_date, 'url': reverse('AudioXApp:creator_request_withdrawal_list')})

        if creator_profile.last_name_change_date and (now - creator_profile.last_name_change_date) < timedelta(days=90):
            recent_activities.append({'type': 'profile_update_name', 'icon_class': 'fas fa-user-edit', 'description': f"Display name changed.", 'timestamp': creator_profile.last_name_change_date, 'url': reverse('AudioXApp:update_creator_profile')})
        ...
        for earning in recent_earning_events:
            if earning.transaction_date:
                ...
                if title_to_display:
                    ...
                    activity_desc += "."
                recent_activities.append({'type': 'earning_received', 'icon_class': 'fas fa-coins', 'description': activity_desc, 'timestamp': earning.transaction_date, 'url': reverse('AudioXApp:creator_my_earnings')})
        ...
        earnings_by_month_qs = CreatorEarning.objects.filter(creator=creator_profile, transaction_date__gte=query_start_date).annotate(month=TruncMonth('transaction_date')).values('month').annotate(total_amount=Sum('amount_earned')).order_by('month')
        ...
        if hasattr(Audiobook, 'created_at'):
            uploads_by_month_qs = Audiobook.objects.filter(creator=creator_profile, created_at__gte=query_start_date).annotate(month=TruncMonth('created_at')).values('month').annotate(count=Count('audiobook_id')).order_by('month')
