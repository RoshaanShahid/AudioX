...
def send_otp(request):
    ...
    - Checks for existing user based on purpose.
...
def login(request):
    ...
    - Logs in user directly if 2FA is not enabled.
    - Returns JSON responses.
    """
    if request.method == 'POST':
...
def verify_login_otp(request):
    """
    Verifies the OTP entered for 2FA login.
...
def forgot_password_request(request):
    ...
    The actual OTP sending is handled by the 'send_otp' view with purpose='password_reset'.
    """
    context = _get_full_context(request)
...
def verify_password_reset_otp(request):
    ...
    If successful, it sets a session flag and prepares for the reset_password_form view.
    """
    try:
        data = json.loads(request.body)
        email = data.get('email')
...
def reset_password_form(request):
    ...
    Requires email and the verified OTP as GET parameters for session validation.
    """
...
def reset_password_confirm(request):
    """
    Processes the new password submission for password reset.
    Requires email, the verified OTP, and new passwords.
    """
    try:
        data = json.loads(request.body)
        email = data.get('email')
