...
def signup(request):
    if request.method == 'POST':
        ...
        phone_number_from_form = request.POST.get('phone')
        ...
        if not phone_number_from_form: errors['phone'] = "Phone number is required."
        ...
        phone_to_save = phone_number_from_form

        try:
            user = User.objects.create_user(email=email, password=password, full_name=full_name, username=username, phone_number=phone_to_save, bio=None)
...
def send_otp(request):
    if request.method == 'POST':
        ...
        try:
            send_mail(subject, message_body_template.format(otp=otp_value), settings.DEFAULT_FROM_EMAIL, [email], fail_silently=False)
...
def reset_password_form(request):
    ...
    if not email or not otp or not verified_email_in_session or not verified_otp_in_session or email.lower() != verified_email_in_session.lower() or otp != verified_otp_in_session:
...
def reset_password_confirm(request):
    ...
    verified_email_in_session = request.session.get('password_reset_verified_email')
    verified_otp_in_session = request.session.get('password_reset_verified_otp')
    if not verified_email_in_session or not verified_otp_in_session or email.lower() != verified_email_in_session.lower() or otp != verified_otp_in_session:
